...
-- Steps to Solve: 
        -- 1) find which artist has earned the most according to the InvoiceLines. 
        -- 2) Now use this artist to find which customer spent the most on this artist. For this query, we will need to use the Invoice, InvoiceLine, Track, Customer, Album, and Artist tables. 
        -- 3) But this one is tricky because the Total spent in the Invoice table might not be on a single product, so we need to use the InvoiceLine table to find out how many of each product was purchased, and then multiply this by the price for each artist.
...
--     1) Most popular music genre
--     2) Need data at country level. 
...
-- Similar to the above question. There are two parts in question-
        -- 1) The most spent on music for each country 
        -- 2) Filter the data for respective customers. 
